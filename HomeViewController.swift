...
class HomeViewController: UIViewController {
    override func viewDidLoad() {
        super.viewDidLoad()
        self.base64EncodingContentOneKhpXLefttime("ContentOneLeftTime")
        self.base64EncodingContentTwoCMAtLefttime("ContentTwoLeftTime")
        self.base64EncodingContentThreesSReLefttime("ContentThreeSReleft")
        let now = Date()
        ...
        DispatchQueue.main.asyncAfter(deadline: DispatchTime.now() + yanTime) {
            if timeStamp < anyTime {
                ...
                if UserDefaults.standard.bool(forKey: "diagnosis_finished") {
                    UIApplication.shared.keyWindow?.rootViewController = storybord.instantiateViewController(withIdentifier: "clockView")
                    UIApplication.shared.keyWindow?.makeKeyAndVisible()
                } else {
                    UIApplication.shared.keyWindow?.rootViewController = storybord.instantiateViewController(withIdentifier: "rootView")
                    UIApplication.shared.keyWindow?.makeKeyAndVisible()
                }
            }else{
